...
if MONDAY_TOKEN and BOARD_DEALS and BOARD_ORDERS and OPENAI_KEY:
    ...
    if 'df_deals' in st.session_state and not st.session_state.df_deals.empty and 'df_orders' in st.session_state and not st.session_state.df_orders.empty:
        ...
        if user_query := st.chat_input("Query datasets..."):
            ...
            if leadership_mode:
                system_role = "You are a C-Suite executive intelligence strategist. Synthesize data into clean markdown tables with bold summaries."
            else:
                system_role = "You are a Meticulous Data Engineering BI Analyst. Clean formatting variations implicitly and map insights cleanly."
            ...
            with st.chat_message("assistant"):
                with st.spinner("Processing..."):
                    try:
                        ...
                        if response.status_code != 200:
                            st.error("OpenRouter connection failure. Check key validity.")
                        else:
